  ...
  useEffect(() => {
    async function handle() {
      try {
        ...
        console.log(
          "[AuthCallback] attempting getSessionFromUrl with location:",
        ...
        if (
          ...
          console.warn(
            "[AuthCallback] supabase.auth.getSessionFromUrl is not available; falling back to manual hash parsing",
          );
          ...
          console.log("[AuthCallback] parsed hash tokens", {
          ...
          if (access_token) {
            if (
              ...
              try {
                ...
                console.error(
                  "[AuthCallback] error calling setSession fallback",
                  setErr,
                );
              }
            } else {
              console.warn(
                "[AuthCallback] supabase.auth.setSession not available; cannot complete manual session set",
              );
            }
          }
        }
        ...
        setMessage(
          "No session found after OAuth. Please try signing in again.",
        );
        console.warn(
          "[AuthCallback] no session found. URL:",
          window.location.href,
        );
      } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-2xl p-8">
      ...
      <div className="mt-4 text-sm text-gray-600">
        If you see this message for more than a few seconds, check the browser
        console and network tab for redirect errors.
      </div>
    </div>
  );
}
