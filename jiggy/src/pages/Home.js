  ...
  useEffect(() => {
    ...
    async function loadThumbs() {
      try {
        ...
        const { data: videos, error } = await supabase.storage.from('assets').list('videos');
        ...
        for (const video of videos) {
          const { data, error: dlError } = await supabase.storage.from('assets').download(`videos/${video.name}`);
          if (dlError) {
